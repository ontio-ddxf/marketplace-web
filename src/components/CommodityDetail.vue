  ...
  <div class="detail_box">
    ...
    <div class="item_box">
      ...
      <div class="item">
        ...
        <img
          v-else
          src="https://ont.io/upload_img/20190420001238_417.png"
          alt
        />
      </div>
      <div class="item">
        <p>{{ $t('common.tag') }}:</p>
        <el-tag v-for="(item, idx) in detailList.data.keywords" :key="idx">{{
          item
        }}</el-tag>
      </div>
      ...
      <div class="item">
        ...
        <!-- 认证状态 -->
        <br />
        <el-tag type="info" v-if="detailList.isCertificated === 0">{{
          $t('common.no_cert')
        }}</el-tag>
    ...
    >
    <el-button type="success" :disabled="signing" v-else round>{{
      $t('common.detail')
    }}</el-button>
  </div>
</template>
    ...
    query() {
      ...
      if (urlSearch.indexOf('?') > -1) {
        ...
        for (var m = 0; m < searchArr.length; m++) {
          searchParamObj[searchArr[m].split('=')[0]] = searchArr[m].split(
            '='
          )[1]
        }
      }
